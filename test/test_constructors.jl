...
@testset "Forecast Constructors" begin
    ...
    #Deterministic Tests
    tDeterministicForecast = PSY.Deterministic(tg,"scalingfactor", Hour(1),DateTime("01-01-01"),24)
    @test tDeterministicForecast isa PowerSystems.Forecast
    tDeterministicForecast = PSY.Deterministic(tg,"scalingfactor", forecast_data)
    @test tDeterministicForecast isa PowerSystems.Forecast
    #Probabilistic Tests
    tProbabilisticForecast = PSY.Probabilistic(tg,"scalingfactor", Hour(1), DateTime("01-01-01"),[0.5, 0.5], 24)
    @test  tProbabilisticForecast isa PowerSystems.Forecast
    tProbabilisticForecast = PSY.Probabilistic(tg,"scalingfactor", [1.0], forecast_data)
    @test  tProbabilisticForecast isa PowerSystems.Forecast
    #Scenario Tests
    tScenarioForecast = PSY.ScenarioBased(tg, "scalingfactor", Hour(1), DateTime("01-01-01"), 2, 24)
    @test  tScenarioForecast isa PowerSystems.Forecast
    tScenarioForecast = PSY.ScenarioBased(tg,"scalingfactor",forecast_data)
    @test  tScenarioForecast isa PowerSystems.Forecast
end
