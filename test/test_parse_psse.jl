...
# Helper to build the minimal branch dict needed by get_branch_type_psse.
...
@testset "Branch-type tolerance normalizations" begin
    ...
    # Exact zero impedance → DiscreteControlledACBranch
    d_zero = _psse_branch_dict(; br_r = 0.0, br_x = 0.0, transformer = false, tap = 0.0)
    @test PowerSystems.get_branch_type_psse(d_zero) == DiscreteControlledACBranch

    # Near-zero impedance within tolerance → DiscreteControlledACBranch
    d_near_zero = _psse_branch_dict(;
        br_r = PowerSystems.ZERO_IMPEDANCE_RESISTANCE_THRESHOLD / 2,
        br_x = PowerSystems.ZERO_IMPEDANCE_REACTANCE_THRESHOLD / 2,
        transformer = false, tap = 0.0,
    )
    @test PowerSystems.get_branch_type_psse(d_near_zero) == DiscreteControlledACBranch
    ...
    # Transformer with X exactly at the threshold (i.e., not strictly near-zero)
    # must NOT be converted — this models real impedance-correction transformers
    d_threshold_tr = _psse_branch_dict(;
        br_r = 0.0, br_x = PowerSystems.ZERO_IMPEDANCE_REACTANCE_THRESHOLD,
        transformer = true, tap = 0.0,
    )
    @test PowerSystems.get_branch_type_psse(d_threshold_tr) != DiscreteControlledACBranch
end
