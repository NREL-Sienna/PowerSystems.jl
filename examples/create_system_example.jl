...
println("="^60)
println("ACBus:                    ", length(collect(get_components(ACBus, sys))))
println("ThermalStandard:          ", length(collect(get_components(ThermalStandard, sys))))
println("Line:                     ", length(collect(get_components(Line, sys))))
println("TwoTerminalGenericHVDCLine: ", length(collect(get_components(TwoTerminalGenericHVDCLine, sys))))
println("PowerLoad:                ", length(collect(get_components(PowerLoad, sys))))
...
for gen in get_components(ThermalStandard, sys)
    println("  $(get_name(gen)): $(get_active_power(gen)) MW at $(get_name(get_bus(gen)))")
end
...
for hvdc in get_components(TwoTerminalHVDC, sys)
    arc = get_arc(hvdc)
    println("  $(get_name(hvdc)): $(get_name(arc.from)) -> $(get_name(arc.to))")
end
