...
branches14 = [
                Line("Line1",  true, @NT(from=nodes14[1],to=nodes14[2]),   0.01938, 0.05917, @NT(from=0.0264, to=0.0264,), 1804.6, @NT(max = 60.0, min = -60.0)),
                Line("Line2",  true, @NT(from=nodes14[1],to=nodes14[5]),   0.05403, 0.22304, @NT(from=0.0246, to=0.0246,), 489.6, @NT(max = 60.0, min = -60.0)),
                Line("Line3",  true, @NT(from=nodes14[2],to=nodes14[3]),   0.04699, 0.19797, @NT(from=0.0219, to=0.0219,), 552.2, @NT(max = 60.0, min = -60.0)),
                Line("Line4",  true, @NT(from=nodes14[2],to=nodes14[4]),   0.05811, 0.17632, @NT(from=0.0,    to=0.0,   ), 605.2, @NT(max = 60.0, min = -60.0)),
                Line("Line5",  true, @NT(from=nodes14[2],to=nodes14[5]),   0.05695, 0.17388, @NT(from=0.0173, to=0.0173,), 614.0, @NT(max = 60.0, min = -60.0)),
                Line("Line6",  true, @NT(from=nodes14[3],to=nodes14[4]),   0.06701, 0.17103, @NT(from=0.0064, to=0.0064,), 611.6, @NT(max = 60.0, min = -60.0)),
                Line("Line7",  true, @NT(from=nodes14[4],to=nodes14[5]),   0.01335, 0.04211, @NT(from=0.0, to=0.0), 2543.4, @NT(max = 60.0, min = -60.0)),
                TapTransformer("Trans3", true, @NT(from=nodes14[4],to=nodes14[7]),  0.0    , 0.20912,  0.0, 0.978,  nothing),
                TapTransformer("Trans1", true, @NT(from=nodes14[4],to=nodes14[9]),  0.0    , 0.55618,  0.0, 0.969,  nothing),
                TapTransformer("Trans2", true, @NT(from=nodes14[5],to=nodes14[6]),  0.0    , 0.25202,  0.0, 0.932,  nothing),
                Line("Line8",  true, @NT(from=nodes14[6],to=nodes14[11]),  0.09498, 0.19890, @NT(from=0.0, to=0.0), 537.3, @NT(max = 60.0, min = -60.0)),
                Line("Line9",  true, @NT(from=nodes14[6],to=nodes14[12]),  0.12291, 0.25581, @NT(from=0.0, to=0.0), 202.0, @NT(max = 60.0, min = -60.0)),
                Line("Line10", true, @NT(from=nodes14[6],to=nodes14[13]),  0.06615, 0.13027, @NT(from=0.0, to=0.0), 445.8, @NT(max = 60.0, min = -60.0)),
                Transformer2W("Trans4", true, @NT(from=nodes14[7],to=nodes14[8]),  0.0      , 0.17615,  0.0,    nothing),
                Line("Line16", true, @NT(from=nodes14[7],to=nodes14[9]),   0.0,     0.11001, @NT(from=0.0, to=0.0), 1244.4, @NT(max = 60.0, min = -60.0)),
                Line("Line11", true, @NT(from=nodes14[9],to=nodes14[10]),  0.03181, 0.08450, @NT(from=0.0, to=0.0), 509.7, @NT(max = 60.0, min = -60.0)),
                Line("Line12", true, @NT(from=nodes14[9],to=nodes14[14]),  0.12711, 0.27038, @NT(from=0.0, to=0.0), 395.9, @NT(max = 60.0, min = -60.0)),
                Line("Line13", true, @NT(from=nodes14[10],to=nodes14[11]), 0.08205, 0.19207, @NT(from=0.0, to=0.0), 769.0, @NT(max = 60.0, min = -60.0)),
                Line("Line14", true, @NT(from=nodes14[12],to=nodes14[13]), 0.22092, 0.19988, @NT(from=0.0, to=0.0), 637.8, @NT(max = 60.0, min = -60.0)),
                Line("Line15", true, @NT(from=nodes14[13],to=nodes14[14]), 0.17093, 0.34802, @NT(from=0.0, to=0.0), 1021.3, @NT(max = 60.0, min = -60.0))
            ]
