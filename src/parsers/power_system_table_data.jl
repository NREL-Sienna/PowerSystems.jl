...
function make_hydro_generator(
    ...
    if gen_type == HydroEnergyReservoir || gen_type == HydroPumpedTurbine
        if !haskey(data.category_to_df, InputCategory.STORAGE)
        ...
        elseif gen_type == HydroPumpedTurbine
            @debug "Creating $(gen.name) as HydroPumpedTurbine" _group =
                IS.LOG_GROUP_PARSING
