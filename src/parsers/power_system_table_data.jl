...
function make_cost(::Type{T}, data, gen, cost_colnames::_HeatRateColumns) where {T<:ThermalGen}
...
function make_cost(::Type{T}, data, gen, cost_colnames::_CostPointColumns) where {T<:ThermalGen}
    cost_pairs = get_cost_pairs(gen, cost_colnames)
...
function make_cost(::Type{T}, data, gen, cost_colnames::_HeatRateColumns) where {T<:HydroGen}
    fuel_price = gen.fuel_price / 1000.0
    cost_pairs = get_cost_pairs(gen, cost_colnames)
    var_cost, fixed = create_pwinc_cost(cost_pairs)
...
function make_cost(::Type{T}, data, gen, cost_colnames::_CostPointColumns) where {T<:HydroGen}
    cost_pairs = get_cost_pairs(gen, cost_colnames)
    var_cost = create_pwl_cost(cost_pairs)
    op_cost = HydroGenerationCost(
        CostCurve(var_cost, UnitSystem.NATURAL_UNITS),
...
function make_cost(::Type{T}, data, gen, cost_colnames::_HeatRateColumns) where {T<:RenewableGen}
...
function make_cost(::Type{T}, data, gen, cost_colnames::_CostPointColumns) where {T<:RenewableGen}
    cost_pairs = get_cost_pairs(gen, cost_colnames)
    var_cost = CostCurve(
