...
function _psse2pm_branch!(pm_data::Dict, pti_data::Dict, import_all::Bool)
    ...
    if haskey(pti_data, "BRANCH")
        for (i, branch) in enumerate(pti_data["BRANCH"])
            ...
            sub_data["source_id"] =
                ["branch", sub_data["f_bus"], sub_data["t_bus"], branch["CKT"]]
            sub_data["index"] = i
...
function _build_switch_breaker_sub_data(
    ...
    sub_data["r"] = branch["R"]
    sub_data["x"] = branch["X"]
    sub_data["state"] = branch["ST"]
    sub_data["active_power_flow"] = 0.0
    sub_data["reactive_power_flow"] = 0.0
    sub_data["rating"] = branch["RATEA"]
    sub_data["discrete_branch_type"] = discrete_branch_type
    sub_data["name"] = "$(device_type == "breaker" ? "BRK" : "SWT")_$(branch["I"])_$(branch["J"])"
    sub_data["source_id"] =
        [device_type, sub_data["f_bus"], sub_data["t_bus"], sub_data["name"]]
    ...
    if haskey(pti_data, "BRANCH")
        for branch in pti_data["BRANCH"]
            ...
            elseif char_init == '*'
                ...
                continue  # Skip branches that aren't switches or breakers
            end
