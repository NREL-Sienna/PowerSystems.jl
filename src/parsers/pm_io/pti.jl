...
function _get_pti_default(
    ...
    transformer = Dict{String,Any}(
        ...
        "VECGRP" => "            ",
        "R1-2" => 0.0,
        "SBASE1-2" => data["CASE IDENTIFICATION"][1]["SBASE"],
        "R2-3" => 0.0,
        "SBASE2-3" => data["CASE IDENTIFICATION"][1]["SBASE"],
        "R3-1" => 0.0,
        "SBASE3-1" => data["CASE IDENTIFICATION"][1]["SBASE"],
        "VMSTAR" => 1.0,
        "ANSTAR" => 0.0,
        "WINDV1" => get(component, "CW", 1) == 2 ?
                    Expr(
            :call,
            :_get_component_property,
            data["BUS"],
    ...
    vsc_dc = Dict{String,Any}(
        ...
        "O1" => Expr(
            ...
            "OWNER",
            "I",
            get(get(component, "CONVERTER BUSES", [Dict()])[1], "IBUS", 0),
        ),
        "O2" => 0,
        "O3" => 0,
        "O4" => 0,
    ...
    gne_device = Dict{String,Any}(
        ...
        "OWNER" => Expr(
            ...
            "OWNER",
            "I",
            get(component, "BUS1", 0),
        ),
        "NMETR" => get(component, "NTERM", 1),
        "REAL" => 0,
        "INTG" => nothing,
        "CHAR" => "1",
    )
    ...
    for (line_number, line) in enumerate(data_lines)
        ...
        elseif length(elements) != 0 && elements[1] == "0" && line_number > 3
            ...
            elseif length(comment) > 0
                @info(
                    "At line $line_number, unexpected section: expected: $section, comment specified: $(section)",
                )
            end
            ...
            elseif section == "VOLTAGE SOURCE CONVERTER"
                ...
                elseif skip_sublines > 0
                    ...
                    try
                        section_data["CONVERTER BUSES"] = append!(
                            section_data["CONVERTER BUSES"],
                            [deepcopy(subsection_data)],
                        )
                    catch message
                        if isa(message, KeyError)
            ...
            for component in data[section]
                ...
                for (field, field_value) in component
                    if isa(field_value, Array)
                        ...
                        for sub_component in field_value
                            ...
                            for (sub_field, sub_field_value) in sub_component
                                if sub_field_value == ""
                                    try
                                        new_sub_component[sub_field] = _get_pti_default(
                                            section,
                                            field,
                                            data,
                                            sub_component;
                    ...
                    elseif field_value == "" &&
                        ...
                        try
                            new_component[field] = _get_pti_default(
                                section,
                                field,
                                data,
                                component,
