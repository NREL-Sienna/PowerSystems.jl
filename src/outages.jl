...
- `monitored_components::Set{Int}` — integer ids of devices whose
...
# Public API for monitored_components accepts integer ids or Devices interchangeably.
# Devices must already be attached to a system so that they have an assigned id.
_as_id(id::Integer) = Int(id)
_as_id(device::Device) = IS.get_id(device)

"""
Get the set of [`Device`](@ref) integer ids whose post-contingency state should be modeled
...
of `items`. Accepts any iterable whose elements are integer ids or
[`Device`](@ref)s (e.g., a `Vector`, a generator, or the iterator returned by
[`get_components`](@ref)). Devices are converted to their integer ids internally and
must already be attached to a system. Pass an empty iterable (or call
[`clear_monitored_components!`](@ref)) to clear the set.
"""
function set_monitored_components!(value::Outage, items)
    empty!(value.monitored_components)
    for x in items
        push!(value.monitored_components, _as_id(x))
    end
...
"""
Add an integer id or [`Device`](@ref) to the monitored-components set of
an [`Outage`](@ref). Adding an existing id is a no-op.
"""
function add_monitored_component!(value::Outage, x::Union{Integer, Device})
    push!(value.monitored_components, _as_id(x))
    return value.monitored_components
end

"""
Add every element of `items` (each an integer id or [`Device`](@ref)) to
the monitored-components set of an [`Outage`](@ref). Accepts any iterable, including
the iterator returned by [`get_components`](@ref). Adding an existing id is a no-op.
"""
...
"""
Remove an integer id or [`Device`](@ref) from the monitored-components set
of an [`Outage`](@ref). No-op when the entry is not present.
"""
function remove_monitored_component!(value::Outage, x::Union{Integer, Device})
    delete!(value.monitored_components, _as_id(x))
    return
end

"""
Remove every element of `items` (each an integer id or [`Device`](@ref)) from
...
- `monitored_components::Set{Int}`: integer ids of devices whose post-contingency state should be modeled when this outage occurs. Empty by default; semantics of an empty set are decided by the downstream consumer.
...
- `monitored_components`: (default: `Int[]`) Any iterable of integer ids or [`Device`](@ref). Devices are converted to their integer ids internally; duplicates are collapsed.
