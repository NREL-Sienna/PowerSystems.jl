...
function PowerLoad(name::String, available::Bool, bus::Bus, maxactivepower::Float64, maxreactivepower::Float64, scalingfactor=nothing)
    scalingfactor=TimeSeries.TimeArray(today(),ones(1)) 
...
PowerLoadPF(; name = "init", available = true, bus = Bus(), maxactivepower = 0.0, power_factor=1.0, 
            scalingfactor=TimeSeries.TimeArray(today(),ones(1))) = PowerLoadPF(name, available, bus, maxactivepower, power_factor, scalingfactor)

PowerLoad(; name = "init", available = true, bus = Bus(), maxactivepower = 0.0, maxreactivepower=0.0, 
            scalingfactor=TimeSeries.TimeArray(today(),ones(1))) = PowerLoad(name, available, bus, maxactivepower, maxreactivepower, scalingfactor)            