...
RenewableFix(; name="init",
        ...
        scalingfactor = TimeSeries.TimeArray(today(),ones(1))) = RenewableFix(name, status, bus, installedcapacity, scalingfactor)
...
RenewableCurtailment(; name = "init",
                ...
                installedcapacity = 0.0,
                econ = EconRenewable(),
                scalingfactor =  TimeSeries.TimeArray(today(),ones(1))) = RenewableCurtailment(name, status, bus, installedcapacity, econ, scalingfactor)
