...
function HydroPumpTurbine(name, available, bus, active_power, reactive_power, rating, active_power_limits, reactive_power_limits, active_power_limits_pump, outflow_limits, powerhouse_elevation, ramp_limits, time_limits, base_power, status=HydroPumpTurbineStatus.OFF, time_at_status=INFINITE_TIME, operation_cost=HydroGenerationCost(nothing), active_power_pump=0.0, efficiency=(turbine = 1.0, pump = 1.0), transition_time=(turbine = 0.0, pump = 0.0), minimum_time=(turbine = 0.0, pump = 0.0), travel_time=nothing, conversion_factor=1.0, must_run=false, prime_mover_type=PrimeMovers.PS, services=Device[], dynamic_injector=nothing, ext=Dict{String, Any}(), )
    _construction_fields = (name = name, available = available, bus = bus, active_power = active_power, reactive_power = reactive_power, rating = rating, active_power_limits = active_power_limits, reactive_power_limits = reactive_power_limits, active_power_limits_pump = active_power_limits_pump, outflow_limits = outflow_limits, powerhouse_elevation = powerhouse_elevation, ramp_limits = ramp_limits, time_limits = time_limits, base_power = base_power, status = status, time_at_status = time_at_status, operation_cost = operation_cost, active_power_pump = active_power_pump, efficiency = efficiency, transition_time = transition_time, minimum_time = minimum_time, travel_time = travel_time, conversion_factor = conversion_factor, must_run = must_run, prime_mover_type = prime_mover_type, services = services, dynamic_injector = dynamic_injector, ext = ext, )
    HydroPumpTurbine(name, available, bus, construct_value(HydroPumpTurbine, _construction_fields, Val(:active_power), Val(:mva)), construct_value(HydroPumpTurbine, _construction_fields, Val(:reactive_power), Val(:mva)), construct_value(HydroPumpTurbine, _construction_fields, Val(:rating), Val(:mva)), construct_value(HydroPumpTurbine, _construction_fields, Val(:active_power_limits), Val(:mva)), construct_value(HydroPumpTurbine, _construction_fields, Val(:reactive_power_limits), Val(:mva)), construct_value(HydroPumpTurbine, _construction_fields, Val(:active_power_limits_pump), Val(:mva)), outflow_limits, powerhouse_elevation, construct_value(HydroPumpTurbine, _construction_fields, Val(:ramp_limits), Val(:mva)), time_limits, base_power, status, time_at_status, operation_cost, construct_value(HydroPumpTurbine, _construction_fields, Val(:active_power_pump), Val(:mva)), efficiency, transition_time, minimum_time, travel_time, conversion_factor, must_run, prime_mover_type, services, dynamic_injector, ext, InfrastructureSystemsInternal(), )
end
...
function from_openapi(po::PO.HydroPumpTurbine, refs::OpenAPIRefs, ::DeviceBaseUnit)
    return HydroPumpTurbine(;
        ...
        active_power_limits_pump = _minmax_from_po(po.active_power_limits_pump),
...
function from_openapi(po::PO.HydroPumpTurbine, refs::OpenAPIRefs, ::NaturalUnit)
    return HydroPumpTurbine(;
        name = po.name,
        available = po.available,
        bus = resolve_ref(refs, po.bus, ACBus),
        ...
        active_power_limits_pump = _minmax_from_po(po.active_power_limits_pump, (/), po.base_power),
        outflow_limits = _minmax_from_po(po.outflow_limits),
        powerhouse_elevation = po.powerhouse_elevation,
...
function to_openapi(value::HydroPumpTurbine, refs::OpenAPIRefs, ::NaturalUnit)
    return PO.HydroPumpTurbine(;
        id = component_id(refs, value),
        name = get_name(value),
        available = get_available(value),
