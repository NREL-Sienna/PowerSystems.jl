...
- `fe_lim::Min_Max`: Upper/Lower limit on frequency error `(fe_min, fe_max)`
- `P_lim::Min_Max`: Upper/Lower limit on power reference `(P_min, P_max)`
...
- `dP_lim::Min_Max`: Upper/Lower limit on power reference ramp rates`(dP_min, dP_max)`
- `P_lim_inner::Min_Max`: Upper/Lower limit on power reference for REECB`(P_min_inner, P_max_inner)`
