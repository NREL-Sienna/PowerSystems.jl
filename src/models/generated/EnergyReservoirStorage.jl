...
function EnergyReservoirStorage(name, available, bus, prime_mover_type, storage_technology_type, storage_capacity, storage_level_limits, initial_storage_capacity_level, rating, active_power, input_active_power_limits, output_active_power_limits, efficiency, reactive_power, reactive_power_limits, base_power, operation_cost=StorageCost(nothing), conversion_factor=1.0, storage_target=0.0, cycle_limits=1e4, ramp_limits=nothing, self_discharge=0.0, standing_loss=0.0, services=Device[], dynamic_injector=nothing, ext=Dict{String, Any}(), )
    _construction_fields = (name = name, available = available, bus = bus, prime_mover_type = prime_mover_type, storage_technology_type = storage_technology_type, storage_capacity = storage_capacity, storage_level_limits = storage_level_limits, initial_storage_capacity_level = initial_storage_capacity_level, rating = rating, active_power = active_power, input_active_power_limits = input_active_power_limits, output_active_power_limits = output_active_power_limits, efficiency = efficiency, reactive_power = reactive_power, reactive_power_limits = reactive_power_limits, base_power = base_power, operation_cost = operation_cost, conversion_factor = conversion_factor, storage_target = storage_target, cycle_limits = cycle_limits, ramp_limits = ramp_limits, self_discharge = self_discharge, standing_loss = standing_loss, services = services, dynamic_injector = dynamic_injector, ext = ext, )
    EnergyReservoirStorage(name, available, bus, prime_mover_type, storage_technology_type, construct_value(EnergyReservoirStorage, _construction_fields, Val(:storage_capacity), Val(:mva)), storage_level_limits, initial_storage_capacity_level, construct_value(EnergyReservoirStorage, _construction_fields, Val(:rating), Val(:mva)), construct_value(EnergyReservoirStorage, _construction_fields, Val(:active_power), Val(:mva)), construct_value(EnergyReservoirStorage, _construction_fields, Val(:input_active_power_limits), Val(:mva)), construct_value(EnergyReservoirStorage, _construction_fields, Val(:output_active_power_limits), Val(:mva)), efficiency, construct_value(EnergyReservoirStorage, _construction_fields, Val(:reactive_power), Val(:mva)), construct_value(EnergyReservoirStorage, _construction_fields, Val(:reactive_power_limits), Val(:mva)), base_power, operation_cost, conversion_factor, storage_target, cycle_limits, construct_value(EnergyReservoirStorage, _construction_fields, Val(:ramp_limits), Val(:mva)), self_discharge, construct_value(EnergyReservoirStorage, _construction_fields, Val(:standing_loss), Val(:mva)), services, dynamic_injector, ext, InfrastructureSystemsInternal(), )
end

function EnergyReservoirStorage(; name, available, bus, prime_mover_type, storage_technology_type, storage_capacity, storage_level_limits, initial_storage_capacity_level, rating, active_power, input_active_power_limits, output_active_power_limits, efficiency, reactive_power, reactive_power_limits, base_power, operation_cost=StorageCost(nothing), conversion_factor=1.0, storage_target=0.0, cycle_limits=1e4, ramp_limits=nothing, self_discharge=0.0, standing_loss=0.0, services=Device[], dynamic_injector=nothing, ext=Dict{String, Any}(), internal=InfrastructureSystemsInternal(), )
    _construction_fields = (name = name, available = available, bus = bus, prime_mover_type = prime_mover_type, storage_technology_type = storage_technology_type, storage_capacity = storage_capacity, storage_level_limits = storage_level_limits, initial_storage_capacity_level = initial_storage_capacity_level, rating = rating, active_power = active_power, input_active_power_limits = input_active_power_limits, output_active_power_limits = output_active_power_limits, efficiency = efficiency, reactive_power = reactive_power, reactive_power_limits = reactive_power_limits, base_power = base_power, operation_cost = operation_cost, conversion_factor = conversion_factor, storage_target = storage_target, cycle_limits = cycle_limits, ramp_limits = ramp_limits, self_discharge = self_discharge, standing_loss = standing_loss, services = services, dynamic_injector = dynamic_injector, ext = ext, )
    EnergyReservoirStorage(name, available, bus, prime_mover_type, storage_technology_type, construct_value(EnergyReservoirStorage, _construction_fields, Val(:storage_capacity), Val(:mva)), storage_level_limits, initial_storage_capacity_level, construct_value(EnergyReservoirStorage, _construction_fields, Val(:rating), Val(:mva)), construct_value(EnergyReservoirStorage, _construction_fields, Val(:active_power), Val(:mva)), construct_value(EnergyReservoirStorage, _construction_fields, Val(:input_active_power_limits), Val(:mva)), construct_value(EnergyReservoirStorage, _construction_fields, Val(:output_active_power_limits), Val(:mva)), efficiency, construct_value(EnergyReservoirStorage, _construction_fields, Val(:reactive_power), Val(:mva)), construct_value(EnergyReservoirStorage, _construction_fields, Val(:reactive_power_limits), Val(:mva)), base_power, operation_cost, conversion_factor, storage_target, cycle_limits, construct_value(EnergyReservoirStorage, _construction_fields, Val(:ramp_limits), Val(:mva)), self_discharge, construct_value(EnergyReservoirStorage, _construction_fields, Val(:standing_loss), Val(:mva)), services, dynamic_injector, ext, internal, )
end
...
get_storage_capacity(value::EnergyReservoirStorage, units) = InfrastructureSystems._strip_units(get_value(value, Val(:storage_capacity), Val(:mva), units))
...
get_storage_capacity_unitful(value::EnergyReservoirStorage, units) = get_value(value, Val(:storage_capacity), Val(:mva), units)
...
get_active_power_unitful(value::EnergyReservoirStorage, units) = get_value(value, Val(:active_power), Val(:mva), units)
...
get_input_active_power_limits(value::EnergyReservoirStorage, units) = InfrastructureSystems._strip_units(get_value(value, Val(:input_active_power_limits), Val(:mva), units))
...
get_input_active_power_limits_unitful(value::EnergyReservoirStorage, units) = get_value(value, Val(:input_active_power_limits), Val(:mva), units)
...
get_output_active_power_limits(value::EnergyReservoirStorage, units) = InfrastructureSystems._strip_units(get_value(value, Val(:output_active_power_limits), Val(:mva), units))
...
get_output_active_power_limits_unitful(value::EnergyReservoirStorage, units) = get_value(value, Val(:output_active_power_limits), Val(:mva), units)
...
get_reactive_power(value::EnergyReservoirStorage, units) = InfrastructureSystems._strip_units(get_value(value, Val(:reactive_power), Val(:mva), units))
...
get_reactive_power_unitful(value::EnergyReservoirStorage, units) = get_value(value, Val(:reactive_power), Val(:mva), units)
...
get_reactive_power_limits(value::EnergyReservoirStorage, units) = InfrastructureSystems._strip_units(get_value(value, Val(:reactive_power_limits), Val(:mva), units))
...
get_reactive_power_limits_unitful(value::EnergyReservoirStorage, units) = get_value(value, Val(:reactive_power_limits), Val(:mva), units)
...
get_ramp_limits(value::EnergyReservoirStorage, units) = InfrastructureSystems._strip_units(get_value(value, Val(:ramp_limits), Val(:mva), units))
...
get_ramp_limits_unitful(value::EnergyReservoirStorage, units) = get_value(value, Val(:ramp_limits), Val(:mva), units)
