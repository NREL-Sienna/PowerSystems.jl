...
- `storage_capacity::Float64`: Maximum storage capacity (can be in units of, e.g., MWh for batteries or liters for hydrogen). When in MWh, this represents the duration in hours the storage can discharge at base_power, validation range: `(0, nothing)`
...
mutable struct EnergyReservoirStorage <: Storage
    ...
    storage_technology_type::StorageTech
    "Maximum storage capacity (can be in units of, e.g., MWh for batteries or liters for hydrogen). When in MWh, this represents the duration in hours the storage can discharge at base_power"
    storage_capacity::Float64
