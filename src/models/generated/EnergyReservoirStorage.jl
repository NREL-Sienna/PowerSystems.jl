...
- `storage_capacity::Float64`: Maximum storage capacity (can be in units of, e.g., MWh for batteries or liters for hydrogen). When in MWh, this represents the energy capacity, which when divided by base_power yields the duration in hours for reference calculations, validation range: `(0, nothing)`
...
mutable struct EnergyReservoirStorage <: Storage
    ...
    storage_technology_type::StorageTech
    "Maximum storage capacity (can be in units of, e.g., MWh for batteries or liters for hydrogen). When in MWh, this represents the energy capacity, which when divided by base_power yields the duration in hours for reference calculations"
    storage_capacity::Float64
