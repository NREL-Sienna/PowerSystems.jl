...
- `dc_current::Float64`: DC current (A) on the converter, validation range: `dc_current_limits`
- `dc_current_limits::MinMax`: Minimum and maximum stable dc current limits (A)
...
mutable struct InterconnectingConverter <: StaticInjection
    ...
    dc_current::Float64
    "Minimum and maximum stable dc current limits (A)"
    dc_current_limits::MinMax
...
function InterconnectingConverter(name, available, bus, dc_bus, active_power, rating, active_power_limits, dc_current, dc_current_limits, base_power, loss_function=LinearCurve(0.0), services=Device[], dynamic_injector=nothing, ext=Dict{String, Any}(), )
    InterconnectingConverter(name, available, bus, dc_bus, active_power, rating, active_power_limits, dc_current, dc_current_limits, base_power, loss_function, services, dynamic_injector, ext, InfrastructureSystemsInternal(), )
end

function InterconnectingConverter(; name, available, bus, dc_bus, active_power, rating, active_power_limits, dc_current, dc_current_limits, base_power, loss_function=LinearCurve(0.0), services=Device[], dynamic_injector=nothing, ext=Dict{String, Any}(), internal=InfrastructureSystemsInternal(), )
    InterconnectingConverter(name, available, bus, dc_bus, active_power, rating, active_power_limits, dc_current, dc_current_limits, base_power, loss_function, services, dynamic_injector, ext, internal, )
end
...
function InterconnectingConverter(::Nothing)
    InterconnectingConverter(;
        ...
        dc_current=0.0,
        dc_current_limits=(min=0.0, max=0.0),
        base_power=0.0,
...
"""Get [`InterconnectingConverter`](@ref) `dc_current_limits`."""
get_dc_current_limits(value::InterconnectingConverter) = value.dc_current_limits
...
"""Set [`InterconnectingConverter`](@ref) `dc_current_limits`."""
set_dc_current_limits!(value::InterconnectingConverter, val) = value.dc_current_limits = val
