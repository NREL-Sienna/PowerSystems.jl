...
function InterconnectingConverter(name, available, bus, dc_bus, active_power, rating, active_power_limits, base_power, reactive_power_limits=nothing, dc_current=0.0, max_dc_current=1e8, loss_function=LinearCurve(0.0), dc_control=VSCDCControlModes.DC_VOLTAGE, ac_control=VSCACControlModes.AC_REACTIVE_POWER, dc_setpoint=0.0, ac_setpoint=1.0, dc_voltage_droop=0.0, remote_bus_control=nothing, rmpct=100.0, power_factor_weighting_fraction=1.0, voltage_limits=(min=0.0, max=999.9), services=Device[], dynamic_injector=nothing, ext=Dict{String, Any}(), )
    _construction_fields = (name = name, available = available, bus = bus, dc_bus = dc_bus, active_power = active_power, rating = rating, active_power_limits = active_power_limits, base_power = base_power, reactive_power_limits = reactive_power_limits, dc_current = dc_current, max_dc_current = max_dc_current, loss_function = loss_function, dc_control = dc_control, ac_control = ac_control, dc_setpoint = dc_setpoint, ac_setpoint = ac_setpoint, dc_voltage_droop = dc_voltage_droop, remote_bus_control = remote_bus_control, rmpct = rmpct, power_factor_weighting_fraction = power_factor_weighting_fraction, voltage_limits = voltage_limits, services = services, dynamic_injector = dynamic_injector, ext = ext, )
    InterconnectingConverter(name, available, bus, dc_bus, construct_value(InterconnectingConverter, _construction_fields, Val(:active_power), Val(:mva)), construct_value(InterconnectingConverter, _construction_fields, Val(:rating), Val(:mva)), construct_value(InterconnectingConverter, _construction_fields, Val(:active_power_limits), Val(:mva)), base_power, construct_value(InterconnectingConverter, _construction_fields, Val(:reactive_power_limits), Val(:mva)), construct_value(InterconnectingConverter, _construction_fields, Val(:dc_current), Val(:mva)), construct_value(InterconnectingConverter, _construction_fields, Val(:max_dc_current), Val(:mva)), loss_function, dc_control, ac_control, dc_setpoint, ac_setpoint, dc_voltage_droop, remote_bus_control, rmpct, power_factor_weighting_fraction, voltage_limits, services, dynamic_injector, ext, InfrastructureSystemsInternal(), )
end
