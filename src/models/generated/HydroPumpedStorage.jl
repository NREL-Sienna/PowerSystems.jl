...
- `storage_target::UpDown`: (default: `(up=1.0, down=1.0)`) (optional) Storage target of upper reservoir at the end of simulation as ratio of storage capacity.
- `operation_cost::Union{HydroGenerationCost, StorageCost, MarketBidCost}`: (default: `HydroGenerationCost(nothing)`) (optional) Operation Cost of Generation [`OperationalCost`](@ref)
- `pump_efficiency::Float64`: (default: `1.0`) (optional) Pumping effciency [0, 1.0], validation range: `(0, 1)`
- `conversion_factor::Float64`: (default: `1.0`) (optional) Conversion factor from flow/volume to energy: m^3 -> p.u-hr.
- `status::PumpHydroStatus`: (default: `PumpHydroStatus.OFF`) Initial commitment condition at the start of a simulation (`true` = on or `false` = off)
- `time_at_status::Float64`: (default: `INFINITE_TIME`) (optional) Time (e.g., `Hours(6)`) the generator has been generating, pumping, or off, as indicated by `status`
