...
function from_openapi(::Type{ThermalMultiStart}, po, refs::OpenAPIRefs, ::DeviceBaseUnit)
    return ThermalMultiStart(;
        ...
        power_trajectory = (if isnothing(po.power_trajectory); nothing; else; (startup = po.power_trajectory.startup, shutdown = po.power_trajectory.shutdown); end),
        time_limits = (if isnothing(po.time_limits); nothing; else; (up = po.time_limits.up, down = po.time_limits.down); end),
        start_time_limits = (if isnothing(po.start_time_limits); nothing; else; (hot = po.start_time_limits.hot, warm = po.start_time_limits.warm, cold = po.start_time_limits.cold); end),
...
function from_openapi(::Type{ThermalMultiStart}, po, refs::OpenAPIRefs, ::NaturalUnit)
    return ThermalMultiStart(;
        ...
        power_trajectory = (if isnothing(po.power_trajectory); nothing; else; (startup = po.power_trajectory.startup / po.base_power, shutdown = po.power_trajectory.shutdown / po.base_power); end),
        time_limits = (if isnothing(po.time_limits); nothing; else; (up = po.time_limits.up, down = po.time_limits.down); end),
        start_time_limits = (if isnothing(po.start_time_limits); nothing; else; (hot = po.start_time_limits.hot, warm = po.start_time_limits.warm, cold = po.start_time_limits.cold); end),
        start_types = po.start_types,
        operation_cost = convert_cost(po.operation_cost),
