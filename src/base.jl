...
function find_connected_components(sys::PSY.System)
    a = PNM.Adjacency(sys; check_connectivity = false)
    return PNM.find_connected_components(a.data, a.lookup[1])
end
...
function validate_connectivity(
    ...
    nodes = sort!(
        collect(
            PSY.get_components(x -> PSY.get_bustype(x) != BusTypes.ISOLATED, Bus, sys),
        );
