...
"""`minimum_energy_offer`'s own wire type — see `convert_cost(vc::PC.TimeSeriesInputOutputCurve2, ...)`
in `cost_conversion.jl` for why it is a separate Julia type from the bare
`TimeSeriesInputOutputCurve` above."""
function _minimum_energy_offer_to_openapi(curve::TimeSeriesLinearCurve)
...
"""`shut_down`'s own wire type — see `_minimum_energy_offer_to_openapi`."""
...
function convert_cost_to_openapi(cost::MarketBidTimeSeriesCost)
    ...
    return PC.MarketBidTimeSeriesCost(;
        minimum_energy_offer = _minimum_energy_offer_to_openapi(
            get_minimum_energy_offer(cost),
        ),
