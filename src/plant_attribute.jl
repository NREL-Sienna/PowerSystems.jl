...
function _group_indices(group_map::AbstractDict, uuid::Base.UUID)
    return sort!([index for (index, uuids) in group_map if uuid in uuids])
end

"""Whether `uuid` appears anywhere in `group_map`."""
function _in_group_map(group_map::AbstractDict, uuid::Base.UUID)
...
function _reverse_multi_group_map(group_map::AbstractDict)
    reverse_map = Dict{Base.UUID, Vector{Int}}()
...
function _drop_from_group_map!(group_map::AbstractDict, uuid::Base.UUID, indices)
...
function _push_to_group_map!(group_map::AbstractDict, uuid::Base.UUID, index::Int)
    push!(get!(group_map, index, Base.UUID[]), uuid)
    return
end
