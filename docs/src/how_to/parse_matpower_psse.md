...
In PowerSystems v5, we have implemented the following conventions for parsing PSSe files:
  ...
  - **Reading and Storing Transformer Data**: The transformer data is always stored in the devices
    ' base. See [Transformer per unit transformations](@ref transformers_pu_per_unit) for additional details.
  ...
  - **Reading [`GeographicInfo`](@ref) data from substations (PSSe v35 only)**: If the file contains a substation section, coordinates are automatically loaded as a [`GeographicInfo`](@ref) supplemental attribute and assigned to the relevant buses. See [Supplemental attributes](@ref supplemental_attributes_explanation).
