...
There is a nuanced distinction between a slack bus and a reference bus. In most small test systems and academic discussions, the system has a single slack bus which is also the reference bus. However, for large interconnected cases or cases with a very radial structure, having a single bus that takes on all the real power mismatch in the system can lead to erroneous results. In PowerSystems.jl we distinguish the possibility of having slacks and reference buses. Is up to the modeler to decide how to handle the classifications inside of the applications. In other words, wether a reference bus is also a slack or vice versa is left to the application developer.
